...
function setup() {
  ...
  let mLabel = createP();
  mLabel.html("Block Movement: ");
  mLabel.parent("canvas-container");

  let mInput;
  if (window.p3_worldMovement) {
    mInput = createInput(window.p3_worldMovement());
  } else {
    mInput = createInput();
  }
  mInput.parent(mLabel);
  mInput.input(() => {
    if (window.p3_changeMovement) {
      window.p3_changeMovement(mInput.value());
    }
  });

  createP("Arrow keys scroll. Clicking drops a block that will start moving.").parent("canvas-container");
  createP("Options for Block Movement: Hash, Random, Up, Down, Left, Right").parent("canvas-container");
