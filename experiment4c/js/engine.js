...
function setup() {
  ...
  let mInput;
  mInput = createInput(100);
  mInput.parent(mLabel);
  mInput.input(() => {
    if (window.p3_adjustTileDimensions) {
      window.p3_adjustTileDimensions(mInput.value());
    }
  });

  createP("Arrow keys scroll. Clicking drops a tower that's impervious to the waves.").parent("canvas-container");
  createP("FOR THE SAKE OF YOUR COMPUTER, DON'T MESS WITH THE ZOOM TO BE ANY SMALLER").parent("canvas-container");
